...
import { Breadcrumb, Layout, Space, Menu, Input, theme, Avatar, Button, Table, Pagination, message } from 'antd';
...
function Prime() {
  ...
  const [searchtxt, setsearchtxt] = useState(''); /* 搜索文本 */
  ...
  const onSearch = () => {
    if (searchtxt == '') {
      ...
      fetch(`https://123.249.122.174:9999/manager/info?username=${searchtxt}`)
        .then(response => response.json())//解析为可读数据
        .then(data => {
          console.log(data)
          setlist([data.data.aa])
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <div style={{paddingLeft:30}}>是</div>
        </Space>
      ),
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          >
            {/* <img src={record.photoAddress} alt="" style={{ width: '100px', height: '100px' }} /> */}
